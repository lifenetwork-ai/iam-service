...
type IdentityOrganization struct {
	...
	SelfAuthenticate bool                  `json:"self_authenticate" gorm:"not null;default:false"`           // Self-authentication flag
	AuthenticateUrl  string                `json:"authenticate_url"`                                          // Profile API path
...
func (m *IdentityOrganization) ToDTO() dto.IdentityOrganizationDTO {
	return dto.IdentityOrganizationDTO{
		...
		Name:             m.Name,
		Code:             m.Code,
		SelfAuthenticate: m.SelfAuthenticate,
